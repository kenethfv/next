export const generatePaginationNumbers = (
  ...
  totalPages: number
    ...
    // vamos a mostrar todas las paginas si npuntos suspensivos
    if (totalPages <= 7) {
        return Array.from({ length: totalPages }, (_, i) => i + 1); // [1, 2, 3, 4, 5, 6, 7]
    }

    // Si la pagina actual esta entre las primeras 3 paginas
    // mostar las primeras 3, puntos suspensivos y las ultimas 2
    if (currentPage <= 3) {
    ...
    return [1, "...", currentPage - 1, currentPage, currentPage + 1, "...", totalPages]; // [1, "...", 30, 31, 32, "...", 50]
